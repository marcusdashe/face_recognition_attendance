...
class App:
    def __init__(self):
        ...
        self.main_window.geometry("1200x520+300+50")
        self.main_window.title(
            "Timnang Final Year Project | Biometric Attendance System")
        self.login_button_main_window = util.get_button(
            self.main_window, 'Mark Attendance', 'green', self.login)
    ...
    def login(self):
        ...
        output = subprocess.run(
            ['face_recognition', self.db_dir, unknown_img_path], capture_output=True, text=True, check=True).stdout

        name = output.split(',')[-1].strip()
        if name in ['unknown_person', 'no_person_found']:
            util.msg_box(
                'ooops!....', 'Unknown user. Please register new user or try again')
        else:
        ...
        #     model_dir=r'C:\Users\hp\Desktop\Timnan\timnang_biometric_attendance_project_v3',
        #     device_id=0
        # )
    ...
    def logout(self):
        ...
        if label == 1:
            ...
            if name in ['unknown_person', 'no_persons_found']:
                ...
                util.msg_box('Oh Yeah !', 'Goodbye, {}.'.format(name))
                with open(self.log_path, 'a') as f:
                    f.write('{},{},out\n'.format(
    ...
    def register_new_user(self):
        ...
        self.try_again_button_register_new_user_window = util.get_button(
            self.register_new_user_window, 'Try again', 'orange', self.try_again_register_new_user)
